...
#r "nuget: Mscc.GenerativeAI, 1.5.0"
...
const string HashesFilePath = "processed_flash_hashes.json";
...
async Task Main()
{
    ...
    var googleAi = new GoogleAI(apiKeys.GeminiKey);
    var model = googleAi.GenerativeModel(Model.Gemini15FlashLatest);
    ...
    Console.WriteLine("Watching for new or changed .flashq files. Press ENTER to exit.");

    var watcherTask = WatchDirectoryAsync(appConfig.WatchDirectory, model, processedHashes, cts.Token);
...
(ApiKeys apiKeys, AppConfig appConfig) LoadConfigurations()
{
    ...
    try
    {
        ...
        if (string.IsNullOrWhiteSpace(apiKeys.GeminiKey))
        {
            Console.WriteLine("Invalid API keys configuration: GeminiKey must be specified.");
            return (null, null);
        }
    }
    catch (Exception ex)
...
async Task WatchDirectoryAsync(string watchDir, GenerativeModel model, 
    ConcurrentDictionary<string, byte> processedHashes, CancellationToken ct)
{
    ...
    watcher.Filter = "*.flashq";

    watcher.Changed += async (sender, e) => await ProcessFileAsync(e.FullPath, model, processedHashes);
    watcher.Created += async (sender, e) => await ProcessFileAsync(e.FullPath, model, processedHashes);
...
async Task ProcessFileAsync(string filePath, GenerativeModel model,
    ConcurrentDictionary<string, byte> processedHashes)
{
    try
    {
        ...
        if (processedHashes.TryAdd(hash, 1))
        {
            ...
            var config = new GenerationConfig() { Temperature = 0.025f };
            
            var stopwatch = Stopwatch.StartNew();
            var textResponse = await model.GenerateContent(context + "\n\nLatest question: " + question, config);
            stopwatch.Stop();
            
            string answer = textResponse.Text.Trim();
