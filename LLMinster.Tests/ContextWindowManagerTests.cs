...
namespace LLMinster.Tests
{
    public class ContextWindowManagerTests
    {
        ...
        private readonly fsEnsemble.ILanguageModelClient _fakeLanguageModelClient;
        ...
        public ContextWindowManagerTests()
        {
            ...
            _fakeLanguageModelClient = A.Fake<fsEnsemble.ILanguageModelClient>();
            _contextWindowManager = new ContextWindowManager(_fakeEventStore, _fakeLanguageModelClient);
        }

        [Fact(DisplayName = "ReconstructWindowAsync should return a properly formatted context window from event history")]
        public async Task ReconstructWindowAsync_ShouldReturnFormattedContextWindow()
        {
            ...
            var events = new List<SessionEvent>
            {
                ...
                new SessionEvent { SessionId = sessionId, ModelName = "User", Content = "How are you?", SequenceNumber = 3 }
            };
        ...
        public async Task ProcessMessageAsync_ShouldAppendUserMessageAndReturnAIResponse()
        {
            ...
            var aiResponse = "I'm sorry, but as an AI language model, I don't have access to real-time weather information. You might want to check a weather website or app for the most up-to-date information about the weather in your area.";
            var temperature = 1;
            ...
            A.CallTo(() => _fakeLanguageModelClient.GenerateContentAsync(A<fsEnsemble.ContentRequest>._))
                .Returns(Task.FromResult(successResult));

            // Act
            
            var result = await _contextWindowManager.ProcessMessageAsync(sessionId, userMessage, temperature);
