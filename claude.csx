...
const string HashesFilePath = "processed_claude_hashes.json";
...
async Task Main()
{
    ...
    var anthropic = new Anthropic() { ApiKey = apiKeys.ClaudeApiKey };
    ...
    Console.WriteLine("Watching for new or changed .claudeq files. Press ENTER to exit.");

    var watcherTask = WatchDirectoryAsync(appConfig.WatchDirectory, anthropic, appConfig.ClaudeModel, processedHashes, cts.Token);
...
(ApiKeys apiKeys, AppConfig appConfig) LoadConfigurations()
{
    ...
    try
    {
        ...
        if (string.IsNullOrWhiteSpace(apiKeys.ClaudeApiKey))
        {
            Console.WriteLine("Invalid API keys configuration: ClaudeApiKey must be specified.");
            return (null, null);
        }
    }
    catch (Exception ex)
    ...
    try
    {
        ...
        if (string.IsNullOrWhiteSpace(appConfig.WatchDirectory) || string.IsNullOrWhiteSpace(appConfig.ClaudeModel))
        {
            Console.WriteLine("Invalid configuration: WatchDirectory and ClaudeModel must be specified.");
            return (null, null);
        }
    }
    catch (Exception ex)
...
async Task WatchDirectoryAsync(string watchDir, Anthropic anthropic, string modelName,
    ConcurrentDictionary<string, byte> processedHashes, CancellationToken ct)
{
    ...
    watcher.Filter = "*.claudeq";

    watcher.Changed += async (sender, e) => await ProcessFileAsync(e.FullPath, anthropic, modelName, processedHashes);
    watcher.Created += async (sender, e) => await ProcessFileAsync(e.FullPath, anthropic, modelName, processedHashes);
...
async Task ProcessFileAsync(string filePath, Anthropic anthropic, string modelName,
    ConcurrentDictionary<string, byte> processedHashes)
{
    try
    {
        ...
        if (processedHashes.TryAdd(hash, 1))
        {
            ...
            var messages = new[] { new Claudia.Message { Role = "user", Content = context + "\n\nLatest question: " + question } };
            ...
            var response = await anthropic.Messages.CreateAsync(messageRequest);
            stopwatch.Stop();
